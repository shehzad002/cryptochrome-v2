...
const FarmCards: React.FC = () => {
  ...
  const chmIndex = farms.findIndex(
    ({ tokenSymbol }) => tokenSymbol === 'CHM',
  )

  const chmPrice =
    chmIndex >= 0 && stakedValue[chmIndex]
      ? stakedValue[chmIndex].tokenPriceInWeth
      : new BigNumber(0)

  const BLOCKS_PER_YEAR = new BigNumber(2372500)
  const CHM_PER_BLOCK = new BigNumber(60)
...
const FarmCard: React.FC<FarmCardProps> = ({ farm }) => {
  ...
  }, [chm, lpTokenAddress, account, setHarvestable])

  const poolActive = true // startTime * 1000 - Date.now() <= 0

  return (
    <StyledCardWrapper>
      {farm.tokenSymbol === 'CHM' && <StyledCardAccent />}
      <Card>
        <CardContent>
          <StyledContent>
